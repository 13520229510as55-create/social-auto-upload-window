...
async def find_login_qrcode(page: Page, selector: str) -> str:
    ...
    try:
        ...
        # 处理data URI格式（如 data:image/png;base64,iVBORw0KGgo...）
        if login_qrcode_img.startswith("data:image"):
            # 提取base64部分（去掉 data:image/png;base64, 前缀）
            if "," in login_qrcode_img:
                base64_part = login_qrcode_img.split(",", 1)[1]
                utils.logger.info(f"[find_login_qrcode] 从data URI中提取base64，长度: {len(base64_part)}")
                return base64_part
            else:
                utils.logger.warning(f"[find_login_qrcode] data URI格式异常: {login_qrcode_img[:100]}")
