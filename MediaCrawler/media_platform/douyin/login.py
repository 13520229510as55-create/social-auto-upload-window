...
class DouYinLogin(AbstractLogin):
    ...
    async def begin(self):
        ...
        if config.LOGIN_TYPE == "cookie":
            # 使用cookie登录时，直接添加cookie，不需要弹出登录对话框
    ...
    async def login_by_cookies(self):
        ...
        # 确保页面在抖音首页，这样才能正确添加cookie
        current_url = self.context_page.url
        if not current_url.startswith("https://www.douyin.com"):
            utils.logger.info(f"[DouYinLogin.login_by_cookies] 当前URL不是抖音首页，导航到抖音首页: {current_url}")
            await self.context_page.goto("https://www.douyin.com", wait_until='load', timeout=60000)
            await asyncio.sleep(2)  # 等待页面加载
