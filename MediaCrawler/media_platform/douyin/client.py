...
class DouYinClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def request(self, method, url, **kwargs):
        ...
        try:
            ...
            # 添加响应日志，方便调试
            if "/v1/web/general/search" in url:
                utils.logger.debug(f"[DouYinClient.request] 搜索接口响应状态码: {response.status_code}")
                utils.logger.debug(f"[DouYinClient.request] 搜索接口响应keys: {list(response_data.keys()) if isinstance(response_data, dict) else 'N/A'}")
            return response_data
        except Exception as e:
            utils.logger.error(f"[DouYinClient.request] 请求失败: {method} {url}, 错误: {str(e)}, 响应文本: {response.text[:500]}")
