...
class DouYinCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            # 确保导航到抖音首页进行登录
            utils.logger.info(f"[DouYinCrawler] 导航到抖音首页: {self.index_url}")
            ...
            if not await self.dy_client.pong(browser_context=self.browser_context):
                utils.logger.info("[DouYinCrawler] 未检测到登录状态，开始登录流程...")
                # 确保页面在抖音首页
                current_url = self.context_page.url
                if not current_url.startswith("https://www.douyin.com"):
                    utils.logger.info(f"[DouYinCrawler] 当前URL不是抖音首页，重新导航: {current_url}")
                    await self.context_page.goto(self.index_url, wait_until='load', timeout=60000)
                ...
                utils.logger.info("[DouYinCrawler] 已检测到登录状态，跳过登录流程")
            crawler_type_var.set(config.CRAWLER_TYPE)
            if config.CRAWLER_TYPE == "search":
                # 在搜索前，先导航到搜索页面，确保页面状态正确
                # 这样可以确保搜索接口能正常工作
                first_keyword = config.KEYWORDS.split(",")[0].strip() if config.KEYWORDS else ""
                if first_keyword:
                    search_url = f"https://www.douyin.com/search/{first_keyword}?type=general"
                    utils.logger.info(f"[DouYinCrawler] 导航到搜索页面: {search_url}")
                    await self.context_page.goto(search_url, wait_until='load', timeout=60000)
                    await asyncio.sleep(3)  # 等待搜索页面加载完成
                    # 更新cookie，确保搜索接口能使用最新的cookie
                    await self.dy_client.update_cookies(browser_context=self.browser_context)
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * dy_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                try:
                    ...
                    # 添加详细的响应日志
                    utils.logger.info(f"[DouYinCrawler.search] 搜索响应状态码: {posts_res.get('status_code', 'N/A')}")
                    utils.logger.info(f"[DouYinCrawler.search] 搜索响应keys: {list(posts_res.keys())}")
                    if "data" in posts_res:
                        data_list = posts_res.get("data", [])
                        utils.logger.info(f"[DouYinCrawler.search] 搜索返回数据条数: {len(data_list) if isinstance(data_list, list) else 'N/A'}")
                        if isinstance(data_list, list) and len(data_list) > 0:
                            utils.logger.info(f"[DouYinCrawler.search] 第一条数据类型: {type(data_list[0])}, keys: {list(data_list[0].keys())[:5] if isinstance(data_list[0], dict) else 'N/A'}")
                    else:
                        utils.logger.warning(f"[DouYinCrawler.search] 响应中没有'data'字段，完整响应: {str(posts_res)[:500]}")
                    
                    if posts_res.get("data") is None or posts_res.get("data") == []:
                        utils.logger.warning(f"[DouYinCrawler.search] search douyin keyword: {keyword}, page: {page} is empty, 响应: {str(posts_res)[:300]}")
                        # 检查是否有错误信息
                        if "error" in posts_res or "err_no" in posts_res:
                            error_msg = posts_res.get("error", posts_res.get("err_no", ""))
                            utils.logger.error(f"[DouYinCrawler.search] 搜索接口返回错误: {error_msg}")
                        # 不立即break，尝试重试或继续
                        await asyncio.sleep(2)
                        # 如果第一页就是空的，可能是登录问题或关键词问题
                        if page == start_page:
                            utils.logger.error(f"[DouYinCrawler.search] 第一页搜索为空，可能是登录状态问题或关键词无效")
                        break
                except DataFetchError as e:
                    utils.logger.error(f"[DouYinCrawler.search] search douyin keyword: {keyword} failed, 错误: {str(e)}")
                    break
                except Exception as e:
                    utils.logger.error(f"[DouYinCrawler.search] search douyin keyword: {keyword} 发生未知错误: {str(e)}")
                    import traceback
                    utils.logger.error(f"[DouYinCrawler.search] 错误堆栈: {traceback.format_exc()}")
                    break

                page += 1
                if "data" not in posts_res:
                    utils.logger.error(f"[DouYinCrawler.search] search douyin keyword: {keyword} failed，响应中没有data字段，账号也许被风控了。完整响应: {str(posts_res)[:500]}")
                    break
