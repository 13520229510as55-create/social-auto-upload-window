...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def login_by_cookies(self):
        ...
        # 先获取当前浏览器中的cookie（未添加cookie之前的状态）
        # 这与qrcode和mobile登录的逻辑一致
        current_cookie = await self.browser_context.cookies()
        _, current_cookie_dict = utils.convert_cookies(current_cookie)
        no_logged_in_session = current_cookie_dict.get("web_session", "")
        ...
        if cookies_to_add:
            ...
            utils.logger.info(f"[XiaoHongShuLogin.login_by_cookies] 已添加 {len(cookies_to_add)} 个cookie")
            # 重新加载页面使cookie生效
            await self.context_page.reload()
            # 等待页面加载和cookie生效
            await asyncio.sleep(2)
            
            # 使用MediaCrawler原有的check_login_state验证登录状态
            # 它会检查web_session是否变化，如果变化则说明登录成功
            try:
                await self.check_login_state(no_logged_in_session)
                utils.logger.info("[XiaoHongShuLogin.login_by_cookies] Cookie登录验证成功")
            except RetryError:
                utils.logger.warning("[XiaoHongShuLogin.login_by_cookies] Cookie登录验证失败，可能需要重新登录")
        else:
            utils.logger.warning("[XiaoHongShuLogin.login_by_cookies] 没有有效的cookie可以添加")
