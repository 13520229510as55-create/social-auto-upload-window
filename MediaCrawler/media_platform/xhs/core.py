...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def search(self) -> None:
        ...
        if not keywords_list:
            utils.logger.error(f"[XiaoHongShuCrawler.search] 关键词为空，无法进行搜索！config.KEYWORDS = '{config.KEYWORDS}'")
            return
        utils.logger.info(f"[XiaoHongShuCrawler.search] 准备搜索关键词列表: {keywords_list}")
